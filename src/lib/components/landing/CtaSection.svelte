<script lang="ts">
	import { Pickaxe, BotMessageSquareIcon, Bug, CodeXml, CloudUploadIcon, GitGraph } from 'lucide-svelte';
	import Marquee from '../magic/marquee/Marquee.svelte';
	import CtaCard from './CtaCard.svelte';
	let tiles: { icon: any; bg: string }[] = [
		{
			icon: GitGraph,
			bg: 'pointer-events-none absolute left-1/2 top-1/2 size-1/2 -translate-x-1/2 -translate-y-1/2 overflow-visible rounded-full bg-gradient-to-r from-purple-500 via-yellow-500 to-green-500 opacity-70 blur-[20px]'
		},
		{
